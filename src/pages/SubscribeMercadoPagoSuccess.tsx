...
import { apiConfirmMercadoPagoSubscription } from "@/lib/api";
import { useAuth } from "@/contexts/AuthContext";

type Status = "loading" | "success" | "error";

const SubscribeMercadoPagoSuccess = () => {
  const [params] = useSearchParams();
  const navigate = useNavigate();
  const { isAuthenticated, authLoading, refreshUser } = useAuth();
  const [status, setStatus] = useState<Status>("loading");
  const [message, setMessage] = useState("Estamos confirmando tu suscripción con Mercado Pago...");
  ...
  useEffect(() => {
    ...
    if (!subscriptionId) {
      setStatus("error");
      setMessage("No encontramos el identificador de suscripción en el retorno de Mercado Pago.");
      return;
    }

    let alive = true;

    apiConfirmMercadoPagoSubscription(subscriptionId)
      .then(async (result) => {
        if (!alive) return;
        localStorage.setItem("astar_mp_subscription_id", subscriptionId);
        await refreshUser();
        ...
        setStatus("error");
        setMessage("Mercado Pago devolvió un estado no activo. Intenta nuevamente o contáctanos.");
      })
      .catch((error: unknown) => {
        if (!alive) return;
...
export default SubscribeMercadoPagoSuccess;
